            ...
            try:
                ...
                if user:
                    ...
                    bot.send_message(
                        ...
                        f"1.2. Банк 'Essence Bank' (далее — Банк) предоставляет игрокам возможность хранения, обмена игровой валюты на сервере.\n"
                        f"1.3. Администрация Сервера вправе изменять условия оферты, уведомляя игроков через Discord.\n"
                    )
                    ...
                    bot.send_message(
                        ...
                        f"3.1. Пополнение:\n"
                        f"• Внесение средств происходит только через кассу Банка.\n"
                        f"3.2. Снятие:\n"
                        f"• Снятие так же происходит только через кассу банка.\n"
                        f"3.3. Переводы:\n"
                        f"• Возможность перевода средств другим игрокам.\n"
                        f"• При переводе игрокам может взыматься комиссия в размере 5% за счёт отправителя.\n"
                    )
                    ...
                    bot.send_message(
                        ...
                        f"• Мошенничество вне банка (например, обман при личной передаче валюты, банк лишь может поспособствовать расследованию).\n"
                        f"• Случайные переводы и взымание комиссии за них.\n"
                        f"4.2. Заморозка счёта.\n"
                        f"• При нарушении правил сервера (читы, дюпы) счёт может быть заморожен.\n"
                        f"\n"
